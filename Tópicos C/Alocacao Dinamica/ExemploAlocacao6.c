/*
    Escreva um programa que leia primeiro os 6 números gerados pela loteria
    e depois os 6 números do seu bilhete.
...
int main()
{
    ...
    int *sorteio;                        // para alocao do vetor dos numeros acertados
    ...
    while (i < 6) // sorteia numeros aleatorios para a loteria
    {
        ...
        for (int j = 0; j < 6; j++)
        {
            // erro se numero == 0 ou ja existir na loteria
            if (sort == loteria[j] || sort == 0)
            {
    ...
    while (i < 6) // lendo os numeros apostados
    {
        ...
        for (j = 0; j < 6; j++)
        {
            // verifica so o numero == 0 ou se ja foi apostado
            if (numero == bilhete[j] || numero == 0)
            {
    ...
    for (i = 0; i < 6; i++) // verifica os numeros acertados
    {
        for (j = 0; j < 6; j++)
        {
            // compara se o valor do bilhete é igual o da loteria
    ...
    printf("\n");
    printf("Pontuacao: %d", pontos); // imprime a pontuacao

    free(sorteio); // desalocacao
