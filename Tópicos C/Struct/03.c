/*
3. Faça um programa que realize a leitura dos seguintes dados relativos a um conjunto
de alunos: Matricula, Nome, Código da Disciplina, Nota1 e Nota2. Considere uma
turma de até 10 alunos. Após ler todos os dados digitados, e depois de armazenálos
em um vetor de estrutura, exibir na tela a listagem final dos alunos com as suas
...
int main()
{
    ...
    for (int i = 0; i < 10; i++) // leitura dos dados
    {
        ...
        fflush(stdin);
        printf("\n");
        alunos[i].media_final = ponderada(nota1, nota2, media); // chamada da funcao que vai calcular a media ponderada
    }
