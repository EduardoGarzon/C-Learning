...
int main()
{
    ...
    int flagmaior = 2, flagmenor = 1, flagnota1 = 0; // indicam os alunos de acordo com a nota
    double maior = medias[2];                        // armazena o segundo valor das medias
    double menor = medias[1];                        // armazena o terceiro valor das medias
    double maior_nota1 = 0;                          // comparar a maior primeira nota

    // comparando as medias com as armazenadas acima
    for (i = 0; i < 5; i++)
    {
        // comparando a primeira com as duas seguites
        // e a segunda com a terceira ja podemos obter
        // quem é a menor e quem é a maior nota
        if (medias[i] > maior)
        {
            maior = medias[i];
            flagmaior = i;
    ...
    printf("Aluno com a maior nota da primeira prova: %s | Nota: %0.1f\n", aluno[flagnota1].nome, maior_nota1);
