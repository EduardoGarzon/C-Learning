/* Implementar uma lista com insercao ordenada e exlcusao de elementos escolhida pelo usuario */
#include <stdio.h>
#include <stdlib.h>
...
void ordenar(LISTA *lista)
{
    ...
    else
    {
        do
        {
            ...
            {
                if (tmp->num > tmp->prox->num) // se o dado atual for maior que o proximo dado
                {
                    ...
                    cont++;                    // flag incrementa indicando que ouve mudanca
                }
...
void inserir(LISTA *lista) // funcao recebe o ponteiro do inicio
{
    ...
    node *novo = (node *)malloc(sizeof(node)); // alocando novo espaco para *novo do tipo node
    ...
    {
        novo->prox = tmp; // o novo elemento aponta para o primeiro elemento apontado por tmp
        *lista = novo;    // ponteiro inicial da lista aponta para novo
    }
    else
    {
        while (novo->num > tmp->num && tmp->prox != NULL) // percorre a lista ate o final ou ate que o novo seja menor que o proximo
        {
...
void remover(LISTA *lista)
{
    ...
    {
        if ((*lista)->prox == NULL && (*lista)->num == valor) // lista com apenas um elemento
        {
            tmp = *lista;
            (*lista) = (*lista)->prox;
            free(tmp);
