/*
    Criar um programa que realize
    a Multiplicação à Russa
    usando obrigatoriamente funções secundárias.

    A multiplicação à Russa consiste em:
        a) Escrever os números A e B, que se deseja multiplicar
           na parte superior das colunas.
        ...
        d) Somar todos os números da coluna B que estejam
           ao lado de um número ímpar da coluna A.
...
// funcao que multiplicar por 2 os valores de 2 n vezes a quantidade de vezes que A foi dividido
void multiplicar(int B[], int A[])
{
    for (int y = 1; A[y] > 0; y++)
    {
        B[y] = B[y - 1] * 2;
...
// funcao que coloca os valores de B que estao ao lado de um valor impar de A no vetor parcela para soma-los posteriormente.
void soma(int A[], int B[], int parcelas[])
{
    for (int i = 0; A[i] > 0; i++)
    {
        if (A[i] % 2 != 0)
