    ...
    if (ano_total == 0) // condicao: mesmo ano, mes e dia
    {
        if (mes_total == 0)
        {
            if (dia_total == 0)
            {
                ...
                else if ((Hduracao >= 12 && Hduracao <= 23) && (Mduracao >= 0 && Mduracao <= 59)) // acima de 12 e abaixo de 24 horas
                {
...
float tempo(struct estacionamento vagas) // funcao que calcula o tempo permanecido
{
    ...
    // total de dias pela diferenca dos meses no mesmo ano
    if (mes_total > 0 && ano_total == 0)
    {
    ...
    if (ano_total > 0) // anos diferentes
    {
        ...
        // anos diferentes com mes de entrada maior que o mes de saida
        if ((a < b && o > w))
        {
            while (a != b && o != w)
            {
    ...
    do
    {
        ...
        if (opcoes == 0) // estrura que contem as opcoes selecionadas
        {
            while (resposta == 0) // cadastra vagas enquanto a resposta for 0
            {
                do
                {
                    // lendo a vaga desejada
                    printf("Informe o numero da vaga desejada [0 ate 10]: ");
                    scanf("%d", &vaga);
                    fflush(stdin);
        ...
        else if (opcoes == 1) // liberar vagas ocupadas
        {
            while (resposta == 0)
            {
                do
                {
                    // leitura da vaga desejada
                    printf("Informe o numero da vaga para liberar [0 ate 10]: ");
                    scanf("%d", &vaga);
                    fflush(stdin);
                ...
                // informa a vaga ocupada se os status for == 1
                if (vagas[vaga].status == 1)
                {
                    vagas[vaga].status = 0;
